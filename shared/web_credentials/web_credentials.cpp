...
#include "device_config.h" // MAC parse/format helpers (for the Orchestrator-MAC field)

namespace web_credentials
{
  static WebServer *srv = nullptr;
  static bool showMeshFields = false; // set in registerRoutes; nodes show mesh, Orchestrator doesn't
  ...
  static String buildForm()
  {
    ...
    if (showMeshFields)
    {
      ...
      if (device_config::isMeshMode())
        html += " checked";
      html += "> Mesh mode (report to a Orchestrator over ESP-NOW)</label><br><br>";

      String orchestratorVal;
      if (device_config::hasOrchestratorMac())
      {
        uint8_t orchestrator[6];
        device_config::orchestratorMac(orchestrator);
        orchestratorVal = device_config::formatMac(orchestrator);
      }
      html += "<label>Orchestrator MAC<br><input name='orchestratorMac' value='" + orchestratorVal + "' placeholder='AA:BB:CC:DD:EE:FF'></label><br><br>";

      html += "<label><input type='checkbox' name='tgFallback' value='1'";
      if (device_config::telegramFallbackEnabled())
        html += " checked";
      html += "> Telegram fallback if the orchestrator can't be reached</label><br><br>";
    }
  ...
  static void handleSave()
  {
    ...
    // Mesh mode (nodes only): checkboxes are only present in the POST when ticked.
    if (showMeshFields)
    {
      device_config::setMesh(srv->hasArg("meshMode"), srv->hasArg("tgFallback"));
      if (srv->hasArg("orchestratorMac") && srv->arg("orchestratorMac").length() > 0)
      {
        uint8_t orchestratorBytes[6];
        if (device_config::parseMac(srv->arg("orchestratorMac"), orchestratorBytes))
        {
          device_config::setOrchestratorMac(orchestratorBytes);
        }
      }
    }
  ...
  void registerRoutes(WebServer &server, bool showMesh)
  {
    srv = &server;
    showMeshFields = showMesh;
