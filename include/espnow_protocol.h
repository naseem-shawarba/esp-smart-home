...
// a weather node) and the master/gateway. BOTH the node firmware and the master
// include this file so the binary layout matches exactly.
//
// Every message begins with `Header`. The master reads `header.deviceType` to know
// which full struct the bytes are, then casts. Add a new sensor by adding a
// DeviceType + a struct that starts with `Header` — the master just gains one case.
...
namespace espnow_protocol
{
  ...
  struct __attribute__((packed)) Header
  {
    ...
    uint8_t deviceType; // DeviceType — tells the master how to parse the body
    ...
    uint32_t uptimeMs;  // millis() at send (rough ordering; master adds real time)
